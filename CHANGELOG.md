...
- **Deferred (host-element auto-scalers):** `-k auto` / `-kAlpha` / `-host` / `-wcap` — they need a representative host element tag the constraint-emit pass can't yet resolve. Numeric `k` (+ `bipenalty_dtcr`) gives full manual control meanwhile. RBE3 tributary-area `-w` weighting is also still a follow-up.
