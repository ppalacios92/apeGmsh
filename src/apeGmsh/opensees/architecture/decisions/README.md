...
| [0042](0042-render-backend-seam.md) | Rendering-backend seam: `SceneLayer` IR + `RenderBackend` Protocol — render-side mirror of ADR 0026; decouples domain logic from PyVista so `trame` web/Jupyter (and a future ParaView export adapter) drop in behind one seam | Proposed (May 2026; phases R-A → R-C, R-D future) |
...
| [0045](0045-selection-and-pick-contract.md) | Selection & pick contract: `SelectionTarget` IR + `Substrate` + canonical `BBox` + `FilterController` (the `0/1/2/3/4` dim spine) + `SelectableSubstrate` Protocol + serialized `SelectionLog` over an additively-widened `PickBackend` — unifies picking across the model/mesh/results viewers, fixes volume picking + the six-bbox mess, lands above ADR 0044 | Proposed (2026-05-30; from a 13-agent design workflow; 4 open questions; slices S0 → S6) |
