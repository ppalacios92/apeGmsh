...
| **Filtered recorders under stages emit one global region, not per-rank (CORRECT — deck-size only)** — stage-bound MPCO/Ladruno recorders claim through `s.recorder(spec)` and fall through `emit_recorder_spec`'s materialize path, emitting one whole-resolved `region` (single shared tag) in global scope rather than the per-rank INV-4 intersection. This is **correct, not a bug** (investigated 2026-06-18): OpenSees `MeshRegion` silently filters members to each rank's local domain (`setElements/setNodes` guard `if (theEle != 0)`, no abort/warning), so every rank's region is its owned subset and the per-rank files merge by the shared tag. The non-staged INV-4 per-rank emission is a **deck-size optimization** (O(model/np) per rank vs. O(model) global — ADR 0061), not a correctness requirement. | Optional only: route stage filtered recorders through `_plan_partitioned_mpco_recorders` for the same deck-size win. Trigger if O(model) staged region text is ever a measured problem; results are already correct. |
...
| **`s.tied_contact` / `s.mortar` stage-bound claim** — Phase SSI-2.D extension ships nine MP-constraint claim methods on `_StageBuilder` but `s.tied_contact` is omitted: `tied_contact` resolves to a `SurfaceCouplingRecord` whose nested `slave_records: list[InterpolationRecord]` is what emits via the `interpolations()` iterator. `_ExcludeClaimedConstraints` filters on outer-record identity; the nested slaves slip through and would double-emit. `s.mortar` is omitted because `g.constraints.mortar(...)` now resolves to a serial-only `ContactRecord` (ADR 0073 deprecated alias), not a claimable MP `SurfaceCouplingRecord`. | Extend `_ExcludeClaimedConstraints.interpolations()` to filter nested slaves when their parent `SurfaceCouplingRecord` is claimed, or accept per-slave naming as the user contract. See [_DEFERRED.md](_DEFERRED.md) §"`s.tied_contact` / `s.mortar` stage-bound claim". |
