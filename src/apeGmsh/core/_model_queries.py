...
# selection-unification v2 P3-R / §6.3 M-CORRECTION (REVISED): only
# ``Plane`` survives (the retained ``_Queries.plane``); ``Selection``/
# ``_select_impl``/``Line`` were referenced solely by the removed
# ``select``/``select_all*``/``line`` bodies.  The symbol *definitions*
# stay in ``._selection`` (load-bearing for the retained
# ``Selection.select``/``EntitySelection``/P2-G chain).
from ._selection import Plane
