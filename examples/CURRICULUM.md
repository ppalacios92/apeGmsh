...
|---|---|---|---|---|---|---|---|
| 01 | Hello Plate | ADAPT `examples/EOS Examples/archive/plate_basic/plate_basic.ipynb` | The full apeGmsh pipeline: `g.model` → `g.mesh` → `FEMData` → OpenSees → displacement. | geometry primitives, physical groups, `fix`, nodal load, direct `ops.*` ingest, `ops.analyze("Static")` | static linear | Max displacement within 1% of Timoshenko plate theory or analytical beam strip. | — |
| 02 | 2D Cantilever Beam | **NEW** `examples/EOS Examples/curriculum/01-fundamentals/02_cantilever_beam_2D/02_cantilever_beam_2D.ipynb` | Simplest 1D model with analytical check. | beam geometry, `elasticBeamColumn`, point load at free end, tip displacement | static linear | `δ = PL³/(3EI)` within 0.1%. | 01 |
| 03 | Simply-Supported Beam | **NEW** `examples/EOS Examples/curriculum/01-fundamentals/03_simply_supported_beam/03_simply_supported_beam.ipynb` | Distributed load + two supports; midspan displacement + moment. | line load, two pin supports, reaction extraction | static linear | `δ_mid = 5wL⁴/(384EI)` and `M_mid = wL²/8`. | 02 |
| 04 | 2D Portal Frame | **NEW** `examples/EOS Examples/curriculum/01-fundamentals/04_portal_frame_2D/04_portal_frame_2D.ipynb` | First multi-element model; joint moment distribution. | multiple beams, rigid connections, combined loads, reaction + drift | static linear | Hand-calculation via stiffness method; drift within 1%. | 03 |
...
| # | Title | File | Learns | Features | Analysis | Verify | Prereq |
|---|---|---|---|---|---|---|---|
| 05 | Labels and Physical Groups | **NEW** `examples/EOS Examples/curriculum/02-building-blocks/05_labels_and_pgs/05_labels_and_pgs.ipynb` | When to use labels (Tier 1) vs PGs (Tier 2); how both feed load/BC targeting. | `g.model.labels.*`, `g.physical.*`, auto-resolution precedence in `g.loads.point.force(target=)` | static linear | Identical result whether the target is given as a label, a PG, or a raw DimTag list. | 02 |
...
| 07 | Load Patterns and Combinations | **NEW** `examples/EOS Examples/curriculum/02-building-blocks/07_load_patterns/07_load_patterns.ipynb` | Multiple named patterns (`dead`, `live`, `seismic`), `g.loads.pattern()` context manager, combination at analysis time. | `g.loads.pattern()`, multiple `timeSeries`, `LoadPattern` separation in OpenSees | static linear (superposed) | Pattern-wise displacements sum to the combined result within 1e-10. | 01 |
| 08 | Boundary Conditions Walkthrough | **NEW** `examples/EOS Examples/curriculum/02-building-blocks/08_boundary_conditions/08_boundary_conditions.ipynb` | `fix`, `face_sp`, prescribed displacement; homogeneous vs non-homogeneous SPs; when each is correct. | `g.loads.face_sp`, `g.loads.face_load`, `SPRecord`, `fem.nodes.sp` | static linear with prescribed disp | Reaction equals applied prescribed force via FEM work balance. | 03 |
...
| # | Title | File | Learns | Features | Analysis | Verify | Prereq |
|---|---|---|---|---|---|---|---|
| 13 | Beam-to-Solid Coupling | ADAPT `examples/EOS Examples/case_studies/column_to_surface_with_reactions/column_to_surface_with_reactions.ipynb` | 6-DOF master node coupled to 3-DOF surface via phantom nodes + rigid beams. | `g.constraints.node_to_surface`, `NodeToSurfaceRecord`, phantom node tag space | static linear | Reduces to an equivalent beam-theory cantilever stiffness. | 04, 12 |
| 14 | Contact Springs Under a Footing | ADAPT `examples/EOS Examples/case_studies/footing_flexible_slab_on_springs/footing_flexible_slab_on_springs.ipynb` | Soil-structure contact via springs; separation allowed under tension. | `g.constraints.node_to_surface_spring`, broker-side stiff-beam routing, `stiff_beam_groups()` | static nonlinear | Contact pressure redistributes under eccentric load; matches rigid-footing limit analysis. | 13 |
...
| 18 | Lateral-Torsional Buckling | ADAPT `examples/EOS Examples/case_studies/ltb_shell/ltb_shell.ipynb` | Linearized buckling (`EigenAnalysis` with load stiffness), critical-load extraction, mode visualization. | shell elements, geometric stiffness, `ASDShellQ4` corotational | eigen (buckling) | Critical moment within 5% of the standard LTB formula. | 17 |
...
| 20 | Time-History Dynamic Analysis | **NEW** `examples/EOS Examples/curriculum/05-analysis-types/20_time_history/20_time_history.ipynb` | Transient analysis with ground acceleration; Newmark integration; damping. | `MultiSupport` ground motion pattern, Rayleigh damping, `transient` analyze, response history extraction | transient | Peak displacement within 3% of Newmark-integrated closed-form for a SDOF with equivalent mass/stiffness/damping. | 17 |
